...
var rule = {
    ...
    二级: async function (ids) {
        ...
        let gitPublicUrl = 'https://cnb.cool/zhyadc/YsBox/-/git/raw/main/';
        let icon = urljoin(gitPublicUrl, './images/icon_cookie/推送.jpg');
        let vod = {
            vod_pic: icon,
            vod_id: orId,
        ...
        if (input.indexOf('@') > -1) {
            ...
            for (let i = 0; i < list.length; i++) {
                if (/pan.quark|drive.uc|www.ali|cloud.189|yun.139|www.123|pan.baidu/.test(list[i])) {
        ...
        } else if (/pan.quark|drive.uc|www.ali|cloud.189|yun.139|www.123|pan.baidu/.test(input))  {
            if (/pan.quark.cn/.test(input)) {
                playPans.push(input);
                const shareData = Quark.getShareData(input);
                log(`✅shareData的结果: ${JSON.stringify(shareData)}`);
                if (shareData) {
                    const videos = await Quark.getFilesByShareUrl(shareData);
                  //  log(`✅videos的结果: ${JSON.stringify(videos)}`);
                    if (videos.length > 0) {
                        playform.push('Quark-' + shareData.shareId);
                        playurls.push(videos.map((v) => {
                            const list = [shareData.shareId, v.stoken, v.fid, v.share_fid_token, v.subtitle ? v.subtitle.fid : '', v.subtitle ? v.subtitle.share_fid_token : ''];
