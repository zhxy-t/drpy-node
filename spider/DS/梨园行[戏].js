...
var rule = {
    ...
    let {input} = this;
        ...
        let html = await post('https://fly.daoran.tv/API_ROP/play/get/playurl', { headers: rule.headers, body: data });
       // console.log(html);
        let parsed = JSON.parse(html);
        let url = parsed.playUrls.hd;
...
一级: async function () {
    ...
    console.log('input的结果:', input);
        ...
        let html =await post(input, {headers: rule.headers, body: data});
...
二级: async function () {
    let {input} = this;
        ...
        let html =await post('https://fly.daoran.tv/API_ROP/album/res/list', {headers: rule.headers, body: data});
