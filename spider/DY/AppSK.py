...
import re,sys,time,json,base64,hashlib,binascii
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        try:
            ...
            host = ext.get('host') or ext.get('url') or ext.get('site')
            # host = ext['host']
           # self.key = ext['key']
            self.key = ext.get('key') or ext.get('dataKey') or ext.get('datakey')
          #  self.iv = ext['iv']
            self.iv = ext.get('iv') or ext.get('dataIv') or ext.get('dataiv')
            ...
            if not re.match(r'^https?://[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(:\d+)?/?$', host):
                host = self.fetch(host,headers=self.headers).text.strip()
            ...
            response = self.post(f'{self.host}/get_config', data=json.dumps(payload), headers=headers).text
            if response:
                token = self.sk_decrypt(response)
                if token:
                    self.headers['authorization'] = 'Bearer ' + token
            config_ = self.fetch(f'{self.host}/app/config', headers=self.headers).text
    ...
    def homeContent(self, filter):
        ...
        response = self.fetch(f'{self.host}/sk-api/type/list', headers=self.headers).text
    ...
    def homeVideoContent(self):
        response = self.fetch(f'{self.host}/sk-api/vod/list?page=1&limit=12&type=randomlikeindex&area=&lang=&year=&mtype=',headers=self.headers).text
        data_ = self.sk_decrypt(response)
        data = json.loads(data_)['data']
        return {'list': data}

    def categoryContent(self, tid, pg, filter, extend):
        response = self.fetch(f"{self.host}/sk-api/vod/list?typeId={tid}&page={pg}&limit=18&type={extend.get('sort','updateTime')}&area={extend.get('area','')}&lang={extend.get('lang','')}&year={extend.get('year','')}&mtype=&extendtype=", headers=self.headers).text
        data_ = self.sk_decrypt(response)
        data = json.loads(data_)['data']
        return {'list': data, 'page': pg}

    def searchContent(self, key, quick, pg='1'):
        response = self.fetch(f"{self.host}/sk-api/search/pages?keyword={key}&page={pg}&limit=10&typeId=-1", headers=self.headers).text
        data_ = self.sk_decrypt(response)
        data = json.loads(data_)['data']
        return {'list': data, 'page': pg}

    def detailContent(self, ids):
        response = self.fetch(f"{self.host}/sk-api/vod/one?vodId={ids[0]}",headers=self.headers).text
        data_ = self.sk_decrypt(response)
        data = json.loads(data_)['data']
        return {'list': [data]}
    ...
    def playerContent(self, flag, id, vipflags):
        ...
        if direct_json or not (id.startswith('http')) or not (re.match(r'https?:\/\/.*\.(m3u8|mp4|flv)', id)):
            try:
                response = self.fetch(f'{self.host}/sk-api/vod/skjson?url={id}&skjsonindex=0',
                                      headers=self.headers).text
                print(f'{self.host}/sk-api/vod/skjson?url={id}&skjsonindex=0')
                data_ = self.sk_decrypt(response)
                data = json.loads(data_)['data']
                url = data.get('url')
