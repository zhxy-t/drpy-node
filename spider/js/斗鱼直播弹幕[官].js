...
const {Client,getDmHtml} = require('../js/_lib.douyu.cjs');
let room_id;
var rule = {
    title: '斗鱼直播弹幕[官]',
    ...
    searchUrl: '/api/search/liveRoom?#did=10000000000000000000000000001501&limit=20&offset=0&sk=**',
    searchable: 2,
    quickSearch: 0,
    headers: {
    ...
    lazy: async function (flag, id, flags) {
        let { input, hostname,getProxyUrl } = this;
        connectWebSocket(hostname, room_id);
    ...
    推荐: async function() {
        let {input} = this;
        let d = [];
        let jo = JSON.parse(await request(input)).data;
        jo.forEach((it, idex) => {
    ...
    一级: async function() {
        let {input} = this;
        let d = [];
        let jo = JSON.parse(await request(input)).data.list;
        jo.forEach(it => {
            d.push({
                url: it.rid,
                title: it.roomName,
    ...
    二级: async function(ids) {
        let {input} = this;
        room_id = ids[0];
        let playform = ['PC','Mobile','CDN','Proxy']
        let pc_html = ''
        let mobile_html = ''
        let vod = {
            vod_id: ids[0],
            // vod_name: jo.liveData.introduction,
            // vod_content: "🏷分区：虎牙" + "·" + jo.liveData.gameFullName + " 🏷UP主：" + jo.liveData.nick + " 🏷人气：" + jo.liveData.totalCount + (jo.liveStatus === 'ON' ?  " 🏷状态：正在直播":"状态：未开播" )
        }
        try{
            pc_html = await getRoomId(ids[0])
            mobile_html = await getMobileStream(ids[0])
        }catch(e){
            pc_html = await getRoom_Id(ids[0])
        }
        let link_name;
        if (pc_html.data.rtmp_live === undefined) {
            log(`${ids[0]} 房间未开播`);
        } else {
            link_name = pc_html.data.rtmp_live
                .split("?")[0]
                .split(".")[0]
                .split("_")[0];
        }
        let pc_link = 'PC'+'$'+pc_html.data.rtmp_url+'/'+pc_html.data.rtmp_live
        const CDNS = {
        ...
        for (const [cdn, config] of Object.entries(CDNS)) {
            for (const [prefix, format] of Object.entries(config)) {
                const link = `http://${prefix}.${cdn}${link_name}.`;
                if (format.flv) {
                    const flv_link = `${link}flv`;
                    flv.push(prefix+'$'+flv_link);
                }
                if (format.m3u8) {
                    const m3u8_link = `${link}m3u8`;
                    m3u.push(prefix+'$'+m3u8_link);
                }
                console.log("\n");
            }
        ...
        let temp = []
        temp.push(m3u.join('#'))
        let proxy_m3u8 = '代理'+'$'+'https://epg.112114.xyz/douyu/'+ids[0]
        let mobile_m3u8 = 'Mobile'+'$'+mobile_html.data.url
        let playUrls  = []
        playUrls.push(pc_link+'$$$'+mobile_m3u8+'$$$'+temp.join('$$$')+'$$$'+proxy_m3u8)
...
async function getRoomId(roomId){
    ...
    let finalRoomID = 0;
    let reg = new RegExp(/var vdwdae325w_64we.*?function ub98484234\(.*?return eval\(strc\)\(.*?\);\}/,)
    let html = (await getHtml({
        method:'GET',
        url:`https://www.douyu.com/${roomId}`,
    })).data;
    const roomId_html = Number(html.match(/\$ROOM.room_id = ?(\d+);/)[1]);
    if(roomId_html === Number(roomId)){
        console.log("正在开播")
        const signFunc = await getSign(html,roomId,reg,did)
        const ts = Math.floor(new Date().getTime() / 1e3);
        const sign = `${signFunc}(${finalRoomID},"${did}",${ts})`;
        const params = eval(sign);
...
async function getRoom_Id(roomId){
    let did = "10000000000000000000000000001501";
    let finalRoomID = 0;
    let reg = new RegExp(/var vdwdae325w_64we.*?function ub98484234\(.*?return eval\(strc\)\(.*?\);\}/,)
    let html = (await getHtml({
    ...
    if(roomId_html === Number(roomId)){
        ...
        const sign = `${signFunc}(${finalRoomID},"${did}",${ts})`;
        const params = eval(sign);
        let info = await getHtml(`https://playweb.douyu.com/lapi/live/getH5Play/${roomId}?${params}`)
        return info.data;
    }else {
        log("尚未开播")
    }
...
async function getSign(html,roomId,reg,did){
    let ub98484234_ = html.match(reg)[0].replace(/eval\(strc\)\(\w+,\w+,.\w+\);/, "strc;");
    const ts = Math.floor(new Date().getTime() / 1e3);
    const ub98484234Call = `ub98484234(${roomId}, ${did}, ${ts})`;
    let signFunc = "";
    try {
        signFunc = eval(ub98484234_ + ub98484234Call);
    } catch (e) {
        const slices = e.message.split(" ");
        if (slices[slices.length - 1] === "defined") {
            const lossStr = `/var ${slices[0]}=.*?];/`;
            const lossReg = new RegExp(eval(lossStr));
            const matchResult = html.match(lossReg);
            signFunc = eval(ub98484234_ + matchResult[0] + ub98484234Call);
        }
    }
    const v = signFunc.match(/\w{12}/);
    const md5 = CryptoJS.MD5(`${roomId}${did}${ts}${v[0]}`).toString();
...
async function getMobileStream(roomId){
    let did = "10000000000000000000000000001501";
    let finalRoomID = 0;
    let reg = new RegExp(/var vdwdae325w_64we.*?function ub98484234\(.*?return eval\(strc\)\(.*?\);\}/,)
    let html = (await getHtml({
    ...
    if(roomId_html === Number(roomId)){
        ...
        const params = eval(sign);
        let info = await getHtml({
            url:`https://m.douyu.com/hgapi/livenc/room/getStreamUrl`,
            method:'POST',
            data:`${params}&rid=${String(roomId)}&rate=-1`
        })
        return info.data;
    }else {
        log("尚未开播")
...
function connectWebSocket(hostname, room_id) {
    ...
    const client = new Client(room_id);
    // 存储新的WebSocket连接到全局变量
    currentWs = client;
    
    client.on('chatmsg', msg => {
       // console.log(`[弹幕] ${msg.txt}`);
        sendDanmuArray(hostname,`${msg.txt}`);
        //`[${msg.nn}]: ${msg.txt}`
    });
...
function sendDanmuArray(hostname, danmu) {
